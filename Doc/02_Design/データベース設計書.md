...
1. Users (独立)
2. Projects (独立)  
3. UserProjects (Users, Projects依存)
4. Domains (Projects依存)
5. DomainApprovers (Domains, Users依存)
6. FormalUbiquitousLang (Domains, Users依存)
7. DraftUbiquitousLang (Domains, Users, FormalUbiquitousLang依存)
8. RelatedUbiquitousLang (FormalUbiquitousLang依存)
9. DraftUbiquitousLangRelations (DraftUbiquitousLang, FormalUbiquitousLang依存)
10. FormalUbiquitousLangHistory (FormalUbiquitousLang依存)
...
- **外部キー**: UserId → Users.UserId, ProjectId → Projects.ProjectId
...
- **外部キー**: DomainId → Domains.DomainId, ApproverId → Users.UserId
...
| カラム名 | データ型 | 制約 | 説明 |
|---------|----------|------|------|
| HistoryId | BIGINT | PK, IDENTITY | 履歴ID（主キー） |
...
| OccurrenceContext | NVARCHAR(50) | NULL | 発生機会 |
| Remarks | NVARCHAR(500) | NULL | 備考（改行可能） |
| RelatedUbiquitousLangSnapshot | JSON | NULL | 関連ユビキタス言語スナップショット（PostgreSQL:JSONB、SQLite:TEXT） |
| UpdatedBy | BIGINT | NOT NULL | 最終更新者ID |
| UpdatedAt | DATETIME2(7) | NOT NULL | 最終更新日時 |
| IsDeleted | BIT | NOT NULL, DEFAULT 0 | 論理削除フラグ |
...
- **JSON構造**: RelatedUbiquitousLangSnapshotには履歴作成時点での関連ユビキタス言語情報を`{"relatedUbiquitousLangs":[{"relationId":123,"targetUbiquitousLangId":456,"targetJapaneseName":"顧客","targetEnglishName":"Customer","relationType":"related"}]}`形式で保存
- **データ型詳細**: PostgreSQL移行時はJSONB型を使用（検索・インデックス性能向上）、SQLite使用時はTEXT型
