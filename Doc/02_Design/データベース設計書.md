...
**バージョン**: 1.2
**作成日**: 2025-06-29
**最終更新**: 2025-12-10 (Phase Issue79 Step13)
**承認者**: プロジェクトオーナー  
...
6. [ID体系](#6-id体系)
7. [データライフサイクル](#7-データライフサイクル)
8. [インデックス設計](#8-インデックス設計)
...
### 1.3 DB初期化方針（Phase B2 Step7決定）

#### 採用方式
**EF Core Migrations主体・Code First方式**（ADR_023参照）

#### 方針決定の経緯
- **背景**: SQL Scripts（init/*.sql）とEF Migrationsの二重管理による不整合リスク
- **決定**: Entity定義 + EF Migrationsをスキーマ管理の「Source of Truth」とする
- **決定日**: 2025-10-26（Phase B2 Step7）
- **詳細**: `/Doc/07_Decisions/ADR_023_DB初期化方針決定.md`

#### スキーマ管理方式
- **Source of Truth**: C#エンティティ定義 (`src/UbiquitousLanguageManager.Infrastructure/Data/Entities/`)
- **スキーマ変更**: EF Core Migrations（`dotnet ef migrations add`）
- **初期データ**: `DbInitializer.cs`（開発環境）、`InitialDataService.cs`（本番環境スーパーユーザー作成）
- **PostgreSQL固有機能**: Migrations内で`migrationBuilder.Sql()`によるマニュアルSQL実行（COMMENT文、GIN/BRINインデックス）

#### 運用フロー
1. エンティティ定義変更 → Migration作成 → DB適用
2. CHECK制約・COMMENT文: Migration内で`migrationBuilder.Sql()`記述
3. データベース設計書: Migrationと同期して更新（db-schema-management Skill参照）

#### PostgreSQL機能サポート状況
- ✅ **TIMESTAMPTZ**: `timestamp with time zone`型として完全サポート
- ✅ **JSONB**: `HasColumnType("jsonb")`で完全サポート
- ✅ **COMMENT文**: `comment:` パラメータで完全サポート
- ✅ **CHECK制約**: `HasCheckConstraint()`メソッドまたは`migrationBuilder.Sql()`でサポート
- ⚠️ **GIN/BRINインデックス**: `migrationBuilder.Sql()`によるマニュアルSQL必要（Phase C-D対応予定）
...
| カラム名 | データ型 | 制約 | 説明 |
|---------|----------|------|------|
| Id | text | PK | ユーザーID（主キー、GUID文字列） |
| UserName | character varying(256) | NULL | ユーザー名 |
| NormalizedUserName | character varying(256) | NULL | 正規化ユーザー名 |
| Email | character varying(256) | NULL | メールアドレス |
| NormalizedEmail | character varying(256) | NULL | 正規化メールアドレス |
...
| LockoutEnd | timestamp with time zone | NULL | ロックアウト終了日時 |
| LockoutEnabled | boolean | NOT NULL, DEFAULT false | ロックアウト有効フラグ |
| AccessFailedCount | integer | NOT NULL, DEFAULT 0 | アクセス失敗回数 |
| Name | character varying(50) | NOT NULL | ユーザー氏名 |
| IsFirstLogin | boolean | NOT NULL, DEFAULT true | 初回ログインフラグ |
| UpdatedAt | timestamp with time zone | NOT NULL, DEFAULT CURRENT_TIMESTAMP | 最終更新日時 |
| UpdatedBy | character varying(450) | NOT NULL | 最終更新者ID |
| IsDeleted | boolean | NOT NULL, DEFAULT false | 論理削除フラグ |
| InitialPassword | character varying(100) | NULL | 初期パスワード（平文・一時的） |
| PasswordResetToken | text | NULL | パスワードリセットトークン |
| PasswordResetExpiry | timestamp with time zone | NULL | リセットトークン有効期限 |
...
| カラム名 | データ型 | 制約 | 説明 |
|---------|----------|------|------|
| Id | text | PK | ロールID（主キー） |
| Name | character varying(256) | NULL | ロール名 |
| NormalizedName | character varying(256) | NULL | 正規化ロール名 |
| ConcurrencyStamp | text | NULL | 同時実行制御スタンプ |
...
| カラム名 | データ型 | 制約 | 説明 |
|---------|----------|------|------|
| UserId | text | PK, FK | ユーザーID |
| RoleId | text | PK, FK | ロールID |
...
| カラム名 | データ型 | 制約 | 説明 |
|---------|----------|------|------|
| Id | integer | PK | クレームID（主キー） |
| UserId | character varying(450) | NOT NULL, FK | ユーザーID |
...
|---------|----------|------|------|
| Id | integer | PK | クレームID（主キー） |
| RoleId | character varying(450) | NOT NULL, FK | ロールID |
| ClaimType | text | NULL | クレーム種別 |
| ClaimValue | text | NULL | クレーム値 |
...
| カラム名 | データ型 | 制約 | 説明 |
|---------|----------|------|------|
| ProjectId | bigint | PK | プロジェクトID（主キー） |
| ProjectName | character varying(50) | NOT NULL, UNIQUE | プロジェクト名 |
| Description | text | NULL | プロジェクト説明 |
| UpdatedBy | character varying(450) | NOT NULL | 最終更新者ID |
| UpdatedAt | timestamp with time zone | NOT NULL, DEFAULT NOW() | 最終更新日時 |
| IsDeleted | boolean | NOT NULL, DEFAULT false | 論理削除フラグ |
...
| カラム名 | データ型 | 制約 | 説明 |
|---------|----------|------|------|
| UserProjectId | bigint | PK | ユーザープロジェクトID（主キー） |
| UserId | character varying(450) | NOT NULL, FK | ユーザーID |
...
| カラム名 | データ型 | 制約 | 説明 |
|---------|----------|------|------|
| DomainId | bigint | PK | ドメインID（主キー） |
| ProjectId | bigint | NOT NULL, FK | 所属プロジェクトID |
| DomainName | character varying(30) | NOT NULL | ドメイン名 |
...
| ApproverId | character varying(450) | NOT NULL, FK | 承認者ユーザーID |
| UpdatedBy | character varying(450) | NOT NULL | 最終更新者ID |
| UpdatedAt | timestamp with time zone | NOT NULL | 最終更新日時 |
...
| JapaneseName | character varying(30) | NOT NULL | 和名 |
| EnglishName | character varying(50) | NOT NULL | 英名 |
| Description | text | NOT NULL | 意味・説明（改行可能） |
| OccurrenceContext | character varying(50) | NULL | 発生機会 |
...
| DomainId | bigint | NOT NULL, FK | 所属ドメインID |
| JapaneseName | character varying(30) | NOT NULL | 和名 |
| EnglishName | character varying(50) | NULL | 英名 |
| Description | text | NULL | 意味・説明（改行可能） |
| OccurrenceContext | character varying(50) | NULL | 発生機会 |
| Remarks | text | NULL | 備考（改行可能） |
| Status | character varying(20) | NOT NULL, CHECK | 状態（Draft/PendingApproval） |
| ApplicantId | character varying(450) | NULL, FK | 申請者ID |
| ApplicationDate | timestamp with time zone | NULL | 申請日時 |
...
本システムでは以下の2種類のID体系を使用します。

| カテゴリ | 型 | 形式 | 用途 |
|---------|----|----|-----|
| ASP.NET Identity系 | string (text) | GUID文字列 | ユーザー・ロール管理 |
| アプリケーション固有 | bigint | 自動採番 | プロジェクト・ドメイン・ユビキタス言語管理 |

### 6.2 ASP.NET Identity系（GUID文字列）

#### 対象テーブル
- **AspNetUsers**: ユーザー情報（Id: text型）
- **AspNetRoles**: ロール情報（Id: text型）
- **AspNetUserRoles**: ユーザー・ロール関連（UserId, RoleId: text型）
- **AspNetUserClaims**: ユーザークレーム（UserId: text型）
- **AspNetRoleClaims**: ロールクレーム（RoleId: text型）

#### GUID形式規則

| 種別 | 形式 | 例 |
|------|------|-----|
| ユーザーID | `00000000-0000-0000-0000-0000000000XX` | `00000000-0000-0000-0000-000000000001` |
| ロールID | `00000000-0000-0000-0001-0000000000XX` | `00000000-0000-0000-0001-000000000001` |
...
| ロール名 | ロールID |
|---------|---------|
| SuperUser | `00000000-0000-0000-0001-000000000001` |
| ProjectManager | `00000000-0000-0000-0001-000000000002` |
| DomainApprover | `00000000-0000-0000-0001-000000000003` |
| GeneralUser | `00000000-0000-0000-0001-000000000004` |

### 6.3 アプリケーション固有（bigint自動採番）

#### 対象テーブル
- **Projects**: プロジェクト（ProjectId: bigint SERIAL）
- **Domains**: ドメイン（DomainId: bigint SERIAL）
- **UserProjects**: ユーザー・プロジェクト関連（UserProjectId: bigint SERIAL）
- **DomainApprovers**: ドメイン承認者（DomainApproverId: bigint SERIAL）
- **FormalUbiquitousLang**: 正式ユビキタス言語（FormalUbiquitousLangId: bigint SERIAL）
- **DraftUbiquitousLang**: ドラフトユビキタス言語（DraftUbiquitousLangId: bigint SERIAL）
- **RelatedUbiquitousLang**: 関連ユビキタス言語（RelatedUbiquitousLangId: bigint SERIAL）

#### 設計根拠

アプリケーション固有テーブルでbigintを採用した理由：
1. **シンプルな設計**: 自動採番による一意性保証
2. **パフォーマンス**: インデックス効率・結合性能
3. **既存DB設計**: PostgreSQL bigint SERIAL設計を踏襲

### 6.4 設計決定記録

ID体系の詳細な設計根拠・背景については以下を参照：
...
## 10. PostgreSQL識別子規約（2025-10-26確立）

### 10.1 必須ルール: 全識別子Quote必須
...
**詳細**: `.serena/memories/tech_stack_and_conventions.md` - PostgreSQL識別子規約
