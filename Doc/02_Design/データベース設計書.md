...
#### データベース段階移行
- **開発・テスト**: SQLite（軽量、セットアップ簡単）
- **本格運用**: PostgreSQL（部門長承認後移行）

#### 主要な設計原則
- **正規化**: 3NF準拠による冗長性排除
- **参照整合性**: 外部キー制約による整合性確保
- **楽観ロック**: UpdatedAt（最終更新日時）による競合制御
- **監査証跡**: 変更履歴とログ機能による透明性確保
...
erDiagram
    Users {
        ...
        string PasswordHash
        string Name
        enum UserRole "SuperUser/ProjectManager/DomainApprover/GeneralUser"
        boolean IsActive
    ...
    FormalUbiquitousLangHistory {
        ...
        boolean IsDeleted
        datetime ArchivedAt
        bigint ArchivedBy FK
    }
    
    RelatedUbiquitousLangHistory {
        bigint HistoryId PK "技術的"
        bigint RelatedUbiquitousLangId FK
        bigint SourceUbiquitousLangId FK
        bigint TargetUbiquitousLangId FK
        bigint UpdatedBy FK
        datetime UpdatedAt
        datetime ArchivedAt
        bigint ArchivedBy FK
    ...
    Users ||--o{ DomainApprovers : "承認権限"
    Users ||--o{ DraftUbiquitousLang : "編集"
    Users ||--o{ FormalUbiquitousLang : "更新"
    ...
    RelatedUbiquitousLang ||--o{ RelatedUbiquitousLangHistory : "関連履歴"
```

### 4.2 物理ER図の補足説明

#### 主要な制約
- **一意制約**: Email（Users）、ProjectName（Projects）、DomainName（Domains, プロジェクト内）
- **外部キー制約**: 参照整合性を全関連で設定
...
#### 目的
システム利用者の認証・権限情報と基本プロフィールを管理

#### カラム定義

| カラム名 | データ型 | 制約 | 説明 |
|---------|----------|------|------|
| UserId | BIGINT | PK, IDENTITY | ユーザーID（主キー） |
...
| UserRole | NVARCHAR(20) | NOT NULL, CHECK | ユーザーロール（enum） |
| IsActive | BIT | NOT NULL, DEFAULT 1 | アクティブフラグ |
| IsFirstLogin | BIT | NOT NULL, DEFAULT 1 | 初回ログインフラグ |
| PasswordResetToken | NVARCHAR(255) | NULL | パスワードリセットトークン |
| PasswordResetExpiry | DATETIME2(7) | NULL | リセットトークン有効期限 |
| UpdatedBy | BIGINT | NOT NULL, FK | 最終更新者ID |
| UpdatedAt | DATETIME2(7) | NOT NULL | 最終更新日時 |
| IsDeleted | BIT | NOT NULL, DEFAULT 0 | 論理削除フラグ |

#### 制約・ルール
- **一意制約**: Email（削除済み含む）
- **チェック制約**: UserRole IN ('SuperUser', 'ProjectManager', 'DomainApprover', 'GeneralUser')
- **ビジネスルール**: 論理削除時はIsDeleted=1に設定
...
| カラム名 | データ型 | 制約 | 説明 |
|---------|----------|------|------|
| ProjectId | BIGINT | PK, IDENTITY | プロジェクトID（主キー） |
| ProjectName | NVARCHAR(50) | NOT NULL, UNIQUE | プロジェクト名 |
| Description | NVARCHAR(200) | NULL | プロジェクト説明 |
| UpdatedBy | BIGINT | NOT NULL, FK | 最終更新者ID |
| UpdatedAt | DATETIME2(7) | NOT NULL | 最終更新日時 |
| IsDeleted | BIT | NOT NULL, DEFAULT 0 | 論理削除フラグ |
...
- INDEX IX_Projects_CreatedBy (CreatedBy)
...
- **外部キー**: ProjectId → Projects.ProjectId, UpdatedBy → Users.UserId
...
| カラム名 | データ型 | 制約 | 説明 |
|---------|----------|------|------|
| DraftUbiquitousLangId | BIGINT | PK, IDENTITY | ドラフトユビキタス言語ID（主キー） |
| DomainId | BIGINT | NOT NULL, FK | 所属ドメインID |
| JapaneseName | NVARCHAR(30) | NOT NULL | 和名 |
| EnglishName | NVARCHAR(50) | NULL | 英名 |
| Description | NVARCHAR(500) | NULL | 意味・説明（改行可能） |
| OccurrenceContext | NVARCHAR(50) | NULL | 発生機会 |
| Remarks | NVARCHAR(500) | NULL | 備考（改行可能） |
| Status | NVARCHAR(20) | NOT NULL, CHECK | 状態（Draft/PendingApproval） |
| ApplicantId | BIGINT | NULL, FK | 申請者ID |
| ApplicationDate | DATETIME2(7) | NULL | 申請日時 |
| RejectionReason | NVARCHAR(500) | NULL | 却下理由 |
...
- **外部キー**: DomainId → Domains.DomainId, ApplicantId, UpdatedBy → Users.UserId, SourceFormalUbiquitousLangId → FormalUbiquitousLang.FormalUbiquitousLangId
...
| カラム名 | データ型 | 制約 | 説明 |
|---------|----------|------|------|
| FormalUbiquitousLangId | BIGINT | PK, IDENTITY | 正式ユビキタス言語ID（主キー） |
| DomainId | BIGINT | NOT NULL, FK | 所属ドメインID |
| JapaneseName | NVARCHAR(30) | NOT NULL | 和名 |
| EnglishName | NVARCHAR(50) | NOT NULL | 英名 |
| Description | NVARCHAR(500) | NOT NULL | 意味・説明（改行可能） |
| OccurrenceContext | NVARCHAR(50) | NULL | 発生機会 |
| Remarks | NVARCHAR(500) | NULL | 備考（改行可能） |
| UpdatedBy | BIGINT | NOT NULL, FK | 最終更新者ID |
| UpdatedAt | DATETIME2(7) | NOT NULL | 最終更新日時 |
...
- **外部キー**: DomainId → Domains.DomainId, UpdatedBy → Users.UserId
...
| カラム名 | データ型 | 制約 | 説明 |
|---------|----------|------|------|
| UserProjectId | BIGINT | PK, IDENTITY | ユーザープロジェクトID（主キー） |
| UserId | BIGINT | NOT NULL, FK | ユーザーID |
...
- **外部キー**: UserId → Users.UserId, ProjectId → Projects.ProjectId, UpdatedBy → Users.UserId
...
| カラム名 | データ型 | 制約 | 説明 |
|---------|----------|------|------|
| DomainApproverId | BIGINT | PK, IDENTITY | ドメイン承認者ID（主キー） |
| DomainId | BIGINT | NOT NULL, FK | ドメインID |
| ApproverId | BIGINT | NOT NULL, FK | 承認者ユーザーID |
| UpdatedBy | BIGINT | NOT NULL, FK | 最終更新者ID |
| UpdatedAt | DATETIME2(7) | NOT NULL | 最終更新日時 |

#### 制約・ルール
- **複合一意制約**: (DomainId, ApproverId) ドメイン・承認者組み合わせ一意
- **外部キー**: DomainId → Domains.DomainId, ApproverId → Users.UserId, UpdatedBy → Users.UserId
...
| カラム名 | データ型 | 制約 | 説明 |
|---------|----------|------|------|
| RelatedUbiquitousLangId | BIGINT | PK, IDENTITY | 関連ユビキタス言語ID（主キー） |
...
- **外部キー**: SourceUbiquitousLangId, TargetUbiquitousLangId → FormalUbiquitousLang.FormalUbiquitousLangId, UpdatedBy → Users.UserId
...
### 5.9 FormalUbiquitousLangHistory テーブル

#### 目的
正式ユビキタス言語の変更履歴を保持、変更前のFormalUbiquitousLangをそのまま保存

#### カラム定義

| カラム名 | データ型 | 制約 | 説明 |
|---------|----------|------|------|
| FormalUbiquitousLangHistoryId | BIGINT | PK, IDENTITY | 履歴ID（主キー） |
...
| UpdatedAt | DATETIME2(7) | NOT NULL | 最終更新日時 |
| IsDeleted | BIT | NOT NULL, DEFAULT 0 | 論理削除フラグ |
| HistoryCreatedAt | DATETIME2(7) | NOT NULL | 履歴作成日時 |

#### 制約・ルール
- **外部キー**: DomainId → Domains.DomainId, UpdatedBy → Users.UserId
- **ビジネスルール**: FormalUbiquitousLang更新時に変更前データをコピー保存、永続保持（物理削除なし）

#### インデックス
- PRIMARY KEY (FormalUbiquitousLangHistoryId)
- INDEX IX_FormalUbiquitousLangHistory_FormalUbiquitousLangId (FormalUbiquitousLangId)
- INDEX IX_FormalUbiquitousLangHistory_HistoryCreatedAt (HistoryCreatedAt DESC)
- INDEX IX_FormalUbiquitousLangHistory_DomainId (DomainId)

### 5.10 RelatedUbiquitousLangHistory テーブル

#### 目的
ユビキタス言語関連性の変更履歴を保持、変更前のRelatedUbiquitousLangをそのまま保存

#### カラム定義

| カラム名 | データ型 | 制約 | 説明 |
|---------|----------|------|------|
| RelatedUbiquitousLangHistoryId | BIGINT | PK, IDENTITY | 関連履歴ID（主キー） |
| RelatedUbiquitousLangId | BIGINT | NOT NULL | 元の関連ユビキタス言語ID |
...
#### 制約・ルール
- **外部キー**: UpdatedBy → Users.UserId
- **ビジネスルール**: RelatedUbiquitousLang更新・削除時に変更前データをコピー保存、永続保持（物理削除なし）

#### インデックス
- PRIMARY KEY (RelatedUbiquitousLangHistoryId)
- INDEX IX_RelatedUbiquitousLangHistory_RelatedUbiquitousLangId (RelatedUbiquitousLangId)
- INDEX IX_RelatedUbiquitousLangHistory_HistoryCreatedAt (HistoryCreatedAt DESC)
- INDEX IX_RelatedUbiquitousLangHistory_SourceUbiquitousLangId (SourceUbiquitousLangId)

### 5.11 DraftUbiquitousLangRelations テーブル
...
| カラム名 | データ型 | 制約 | 説明 |
|---------|----------|------|------|
| DraftUbiquitousLangRelationId | BIGINT | PK, IDENTITY | ドラフト関連ID（主キー） |
...
- **外部キー**: DraftUbiquitousLangId → DraftUbiquitousLang.DraftUbiquitousLangId, FormalUbiquitousLangId → FormalUbiquitousLang.FormalUbiquitousLangId, UpdatedBy → Users.UserId
...
#### データベース仕様
- **SQLite制約**: ファイルサイズ上限（実用上は問題なし）
- **PostgreSQL移行**: 部門長承認後の段階的移行
- **文字エンコード**: UTF-8対応必須
