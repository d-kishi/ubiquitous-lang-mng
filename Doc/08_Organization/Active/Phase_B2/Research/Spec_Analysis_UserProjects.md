...
#### data-testid属性設計（E2Eテスト対応・Step6前提条件）

**目的**: Playwright E2Eテスト（Phase B2 Step6）でのUI要素特定

**付与対象要素**:

##### Phase A実装済み画面（E2Eテスト経路対応）
| data-testid値 | 対象要素 | 実装画面 | 用途 |
|--------------|---------|---------|------|
| `login-email` | メールアドレス入力フィールド | ログイン画面 | ログイン認証操作 |
| `login-password` | パスワード入力フィールド | ログイン画面 | ログイン認証操作 |
| `login-button` | ログインボタン | ログイン画面 | ログイン実行 |

##### Phase B1実装済み画面（E2Eテスト経路対応）
| data-testid値 | 対象要素 | 実装画面 | 用途 |
|--------------|---------|---------|------|
| `project-list` | プロジェクト一覧テーブル | プロジェクト一覧画面 | 一覧表示検証 |
| `member-button` | 「👥 メンバー」ボタン | プロジェクト一覧画面 | メンバー管理画面遷移テスト |

##### Phase B2新規実装画面
| data-testid値 | 対象要素 | 実装画面 | 用途 |
|--------------|---------|---------|------|
| `user-select` | ユーザー選択ドロップダウン | メンバー管理画面 | メンバー追加操作テスト |
| `add-member-button` | 「✅ 追加」ボタン | メンバー管理画面 | メンバー追加実行テスト |
| `delete-member-button-{userId}` | 各ユーザーの「🗑️」ボタン | メンバー管理画面 | メンバー削除操作テスト |
| `member-list` | メンバー一覧テーブル | メンバー管理画面 | メンバー一覧表示検証 |
| `success-message` | 成功メッセージ表示領域 | 共通 | 成功メッセージ検証 |
| `error-message` | エラーメッセージ表示領域 | 共通 | エラーメッセージ検証 |

**実装方針**:
- **Phase A/B1実装済み画面にも遡ってdata-testid属性を付与**（E2Eテスト経路対応）
- Blazor Serverコンポーネントの各UI要素に`data-testid`属性を付与
- Playwright MCPツールによる要素特定の効率化（75-85%向上）
- Step6前提条件として**必須実装**
