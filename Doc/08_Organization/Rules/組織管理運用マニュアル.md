...
**適用範囲**: 全Phase・全Step
**作成日**: 2025-07-21
**最終更新**: 2025-11-03（Phase B2-F2 TypeScript E2E移行完了反映）  
...
- TypeScript E2Eテスト → e2e-test（Phase B2-F2移行完了）
- 単体テスト → unit-test
...
2. **責務マッピングで SubAgent 選定**
   ...
   - 単体・統合テスト → unit-test / integration-test
   - TypeScript E2Eテスト → e2e-test（Phase B2-F2移行完了）
...
## 🎭 Playwright Test Agents統合運用（2025-11-17追加）
...
**実行フロー**:
```
1. MainAgent → Task(playwright-test-planner): テスト計画生成（該当時）
   - 入力: ユーザー要件・アプリケーションURL
   - 出力: Markdown形式テスト計画書

2. MainAgent → Task(playwright-test-generator): TypeScriptテスト生成（該当時）
   - 入力: Markdown形式テスト計画書
   - 出力: TypeScript *.spec.tsファイル

3. MainAgent → Task(e2e-test): テスト実行・統合検証
   - 入力: 生成されたテストファイル
   - 作業: npx playwright test実行・検証
   - 出力: テスト実行レポート

4. MainAgent → Task(playwright-test-healer): 失敗時の修復（該当時）
   - 条件: テスト失敗時のみ
   - 入力: 失敗テスト名・エラー詳細
   - 出力: 修復済みテストコード
```
...
**効果**: シンプルな実行フロー・コンテキスト使用率低減

### MainAgent実行時の判断基準

**質問**: 新規テストシナリオ作成が必要か？
- **YES** → パターンA（Playwright Test Agents活用）
- **NO** → パターンB（e2e-test Agent単独）

**質問**: テスト失敗時の自動修復が必要か？
- **YES** → パターンA + playwright-test-healer実行
- **NO** → e2e-test Agentで手動修正
...
**❌ 禁止範囲**:
- Playwright Test Agentsの直接呼び出し（SubAgent間呼び出し不可）
- 新規テストシナリオの自動生成（playwright-test-generator責務）

### 関連ドキュメント

- **ADR_024**: E2E専用SubAgent新設決定（SubAgent間呼び出し制限・Playwright Test Agents統合パターン）
- **`.claude/agents/e2e-test.md`**: e2e-test Agent定義（MainAgent調整型統合パターン）
- **`.claude/skills/playwright-e2e-patterns/`**: Playwright E2Eテストパターン詳細
...
## 🆕 PlanMode活用ガイドライン（Claude Code計画ファイル）（2025-11-29追加）

### 概要

Claude CodeのPlanModeで生成される計画ファイル（`~/.claude/plans/`配下）を
組織管理プロセスに統合し、計画策定・技術検討の効率化を図る。

### 1. 基本活用シーン

#### 1.1 Phase/Step開始時の必須使用
**phase-start/step-start時にPlanModeで計画整理を実施**

- **適用タイミング**: 複雑なPhase/Step開始時（3 Stage以上、または複数SubAgent並列実行時）
- **実施内容**:
  - 実装対象スコープの詳細化
  - SubAgent並列実行計画の依存関係整理
  - 技術的リスク・課題の事前洗い出し
- **出力**: 計画ファイル → 結論をStep組織設計書に反映

#### 1.2 技術決定の検討過程記録
**ADR作成前の比較検討を計画ファイルで実施**

- **適用タイミング**: 重要な技術決定が必要な場合
- **実施内容**:
  - 複数オプションの比較表作成
  - メリット・デメリット分析
  - リスク評価・推奨案の整理
- **出力**: 計画ファイル → 結論・根拠をADRに反映

#### 1.3 複雑なタスクの事前計画
**複数Stage/Taskの実行計画整理**

- **適用タイミング**: 3つ以上のタスクを含む作業開始時
- **実施内容**:
  - タスク間依存関係の可視化
  - 実行順序・並列可能性の検討
  - 完了基準の詳細化
- **出力**: 計画ファイル → 作業計画をStep組織設計書に反映

### 2. セッション跨ぎ参照対応

**計画ファイルの永続化が必要な場合のプロセス**

#### 2.1 永続化判断基準
以下のいずれかに該当する場合、計画ファイルをプロジェクト内にコピー保存：
- Phase開始時の計画（Phase全体に影響）
- 複数セッションにわたる作業の計画
- 技術決定の詳細な検討過程（ADR補足資料として）

#### 2.2 永続化手順
1. **セッション終了時**: 計画ファイルの価値を評価
2. **コピー判断**: 永続化基準に該当するか確認
3. **保存先**: `Doc/08_Organization/Active/Phase_XX/Plans/` または `Research/`
4. **ファイル名**: `YYYY-MM-DD_[作業内容]_計画.md`
5. **公式記録との関連付け**: Step組織設計書に計画ファイル参照を記載

### 3. 使用ルール

#### 3.1 必須ルール
- **PlanMode開始**: 複雑な作業開始前に計画ファイルで整理
- **結論の公式記録反映**: 計画ファイルの検討結果は対応する公式ファイルに反映
  - 作業計画 → Step組織設計書
  - 技術決定 → ADR
  - 調査結果 → Research/レポート
- **永続化判断**: セッション終了時に永続化要否を判断

#### 3.2 禁止事項
❌ 計画ファイルを唯一の承認記録として使用（Step組織設計書にも記録必須）
❌ 永続化せずに後続セッション参照を前提とする
❌ 計画ファイルのみで作業完了とする（公式記録への反映必須）

### 4. ADR_016プロセス遵守との両立

- 計画ファイルは**「検討過程」**、公式記録は**「結論・承認」**
- 永続化する場合もプロジェクト内に保存（Git管理可能）
- 承認記録はStep組織設計書に必ず反映

---

**更新履歴**:
- 2025-11-29 PlanMode活用ガイドライン追加（計画ファイルの組織管理プロセス統合）
- 2025-11-03 Phase B2-F2 TypeScript E2E移行完了反映（SubAgent選択ガイド・責務マッピング更新）
