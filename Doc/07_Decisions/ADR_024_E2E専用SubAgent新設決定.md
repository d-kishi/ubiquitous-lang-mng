...
## 🔴 SubAgent間呼び出し制限（2025-11-17追記）

### 技術制約発見

**発見日**: 2025-11-17（Phase B-F2 Step7開始時）
...
- **目的**: 無限ネスティング（infinite nesting）防止
- **設計思想**: フラット階層のみ許可
  - MainAgent → SubAgent: ✅ 可能
  - SubAgent → SubAgent: ❌ 禁止

### e2e-test Agent定義への影響

**問題箇所**（修正前）:
- `.claude/agents/e2e-test.md`行11-12: "Playwright Test Generator/Healer Agent活用（60-70%効率化）"
- 行31-34: "Task toolで`playwright-test-generator` Agent起動"

**技術的評価**: ❌ **実装不可能** - SubAgentはTask toolで他のSubAgentを呼び出せない

### 修正対応（2025-11-17実施）

**修正ファイル**:
1. `.claude/agents/e2e-test.md` - MainAgent調整型統合パターン追記
2. `.claude/commands/subagent-selection.md` - Pattern D注意事項追加
3. `.claude/skills/subagent-patterns/SKILL.md` - Playwright Test Agents統合パターン追記

**新設計方針**: MainAgentオーケストレーション型統合パターン（下記参照）

---

## 🎭 Playwright Test Agents統合パターン（2025-11-17追記）
...
**適用条件**: 新規E2Eテストスイート作成・複雑なシナリオ

**メリット**:
- ✅ Playwright Test Agents機能を最大活用（60-70%効率化）
- ✅ ADR_016プロセス遵守（SubAgent責務分担）
- ✅ MainAgentによる全体調整・品質確認

### パターンB: e2e-test Agent単独実行型
...
**デメリット**:
- ❌ Playwright Test Agents機能未活用

### e2e-test Agentの責務（MainAgent調整型）
...
**策定者**: Claude Code
**承認者**: プロジェクトオーナー
**最終更新**: 2025-11-17（SubAgent間呼び出し制限・Playwright Test Agents統合パターン追記）
