...
## 🔴 CRITICAL: プロジェクト憲法（Project-Specific Constitution）
...
### 🔴 CRITICAL: 修正時の自己検証プロセス（必須）

修正依頼を受けた際は、以下を完了するまで「修正完了」を報告しない：

#### 1. 全体フロー把握
- UI → API → Service → Repository → DB の一連のデータフローを確認
- 修正箇所だけでなく、影響を受ける全コードパスを理解

#### 2. 修正実施
- パッチ的修正ではなく、根本原因に対処

#### 3. 自己検証（以下のいずれかを実行）
- **API修正**: curlでエンドポイント呼び出し、期待結果確認
- **UI修正**: E2Eテスト実行またはブラウザ操作で確認
- **ロジック修正**: 該当する単体テスト実行

#### 4. 報告
- 検証結果を含めて報告
- 「修正しました」ではなく「修正し、[検証方法]で[期待結果]を確認しました」
...
## 仮実装・スタブ登録ルール（Step1.5教訓・2025-12-16追加）

**背景**: Phase B-F3 Step1.5で、Phase A実装時の仮実装が記録されておらず、大規模リファクタ（11h+）が必要となった

### ルール

**仮実装・スタブ作成時の必須アクション**:

1. **コード内にTODOコメント追加**（必須）
   ```csharp
   // TODO: [TECH-XXX] 本実装が必要。理由: XX。対応予定: Phase YY
   ```

2. **GitHub Issue登録**（規模に応じて判断）
   - **小規模（1-2時間以内で対応可能）**: TODOコメントのみでOK
   - **中規模以上（2時間以上 or 複数ファイル影響）**: `tech-debt` ラベル付きIssue登録必須
...
- phase-end Commandの「残課題・仮実装チェック」セクションで棚卸し実施
- 自動検出コマンドで TODO/FIXME/HACK を検索
- 対応判断（即時対応 / 次Phase申し送り / Issue登録）
