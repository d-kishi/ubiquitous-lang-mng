...
### Playwright Test Agents活用指針（2025-11-16確立・2025-11-17更新・Phase B-F2 Step6で検証）

**背景**: Phase B-F2 Step6でPlaywright Test Agents（Planner/Generator/Healer）の効果測定完了

#### Playwright Test Agents配置原則（🔴重要・2025-11-17追加）
...
#### Generator Agent（TypeScript → C#変換）
...
- **例**: パスワード変更テスト後、`/`リダイレクトされる場合は再度`/change-password`へ遷移してリセット実行
...
## VSCode拡張機能更新時の検証プロセス（2025-11-15）
...
## E2Eテスト作成前の前提条件確認プロセス（2025-11-17追加）
...
**再発防止策**:
- 本チェックリスト必須実施
- development_guidelinesに記録（本セクション）
