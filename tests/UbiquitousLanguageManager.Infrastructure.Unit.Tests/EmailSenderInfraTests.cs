...
namespace UbiquitousLanguageManager.Infrastructure.Unit.Tests
{
    ...
    public class EmailSenderTests
    {
        ...
        public EmailSenderTests()
        {
            ...
            _mockConfiguration["App:BaseUrl"].Returns("https://localhost:5001");
        ...
        public async Task SendPasswordResetEmailAsync_正常なパラメータでメール送信成功()
        {
            ...
            // テストのConfiguration設定に合わせたベースURL（https://localhost:5001）
            var expectedResetUrl = $"https://localhost:5001/reset-password?token={Uri.EscapeDataString(resetToken)}&email={Uri.EscapeDataString(toEmail)}";

            // 🎯 モック設定: SMTP接続・認証・送信成功
            ...
            // 🔧 AuthenticateAsyncモック設定追加
            _mockSmtpClient.AuthenticateAsync(
                Arg.Any<string>(),
                Arg.Any<string>(),
                Arg.Any<CancellationToken>())
                .Returns(Task.CompletedTask);
            ...
            // HTMLメールなのでBodyプロパティから取得（TextBodyはHTMLタグが除去されたテキスト）
            var htmlBody = capturedMessage.HtmlBody;
            htmlBody.Should().Contain(expectedResetUrl);
            htmlBody.Should().Contain("24時間");

            // 🔍 モック検証: 接続・認証・送信・切断の順序
            Received.InOrder(() =>
            {
                _mockSmtpClient.ConnectAsync(
                    _smtpSettings.Host,
        ...
        public async Task SendPasswordResetEmailAsync_既に接続済みの場合は再接続しない()
        {
            ...
            // 🚫 再接続しないことを確認（実装ではIsConnectedチェックは無いが、ConnectAsyncが呼ばれることを確認）
            // 注意: 実装を見るとIsConnectedチェックは無いので、常に接続される
            // このテストの意図を保つため、実装に合わせてConnectAsyncが呼ばれることを確認
        ...
        public async Task SendEmailAsync_汎用メール送信メソッドのテスト()
        {
            ...
            // デフォルトでisBodyHtml=trueなのでHtmlBodyを検証
            capturedMessage.HtmlBody.Should().Be(body);
        }
